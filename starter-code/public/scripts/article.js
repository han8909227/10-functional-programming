...
(function(module){
  ...
  Article.numWordsAll = () => {
    return Article.all.map(obj => obj.body)
    .reduce(function(accum, currentVal) {
      return accum + currentVal.split(' ').length;
    },0)
  };
  ...
  Article.allAuthors = () => {
    return Article.all.map(author => author.author).reduce(function(accum, elem, pos, array) {
      if(array.indexOf(elem)===pos){
        accum.push(elem);}
      return accum},[]);
  };
  ...
  Article.numWordsByAuthor = () => {
    return Article.allAuthors().map(function (author) {
      return {author: author, words: Article.all.filter(function(val) {
        ...
        return accum + currentVal.body.split(' ').length
      },0)
      }})
